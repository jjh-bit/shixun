# ===================== 完整糖尿病风险预测模型代码 =====================
# 环境依赖：pip install pandas numpy scikit-learn xgboost lightgbm torch matplotlib seaborn
import numpy as np
import pandas as pd
...
print(df.head())
print("\n标签分布：")
print(df['是否患病'].value_counts())

# ===================== 2. 数据预处理 =====================
# 将业务上不可能为0的特征替换为缺失值
zero_features = ['血糖浓度','血压','皮褶厚度','胰岛素','BMI']
...
model_list.append("随机森林(调优)")

# 绘制特征重要性
feat_imp = pd.Series(rf_best.feature_importances_, index=col_names[:-1])
...
model_list.append("LightGBM(调优)")

# ===================== 8. 可选：深度学习 MLP (PyTorch) =====================
print("\n" + "="*50)
